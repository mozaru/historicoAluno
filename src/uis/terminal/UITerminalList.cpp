...
UITerminalList::UITerminalList(
        const std::vector<Disciplina>& lst_,
        double cr_,
        const std::string& titulo_)
    : lst(lst_), cr(cr_), titulo(titulo_)  {}

void UITerminalList::ajustarJanela() {
...
void UITerminalList::desenharCabecalho(int w) {
    // título centralizado
...
void UITerminalList::desenharRodape(int w, int h) {
    const int qtd = lst.size();
...
void UITerminalList::desenhar() {
    ...
    for (int i = offset; i < fim; ++i) {
        ...
        cout << left
             ...
             << setw(10) << std::fixed << std::setprecision(2) << d.getMedia();
...
bool UITerminalList::processarInput() {
