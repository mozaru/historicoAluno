...
UITerminalForm::UITerminalForm(const std::string& titulo_,
                               const std::vector<CampoFormulario>& campos_)
    : titulo(titulo_), campos(campos_) {}

void UITerminalForm::desenhar() {
    Terminal::clear();
    Terminal::hideCursor();
...
void UITerminalForm::desenharCampo(int idx, int row,
...
void UITerminalForm::desenharBotoes(int row, int colCentro) {
    // [ OK ] [ Cancelar ]
    std::string okStr = "[ OK ]";
...
void UITerminalForm::moverFocoCima() {
...
void UITerminalForm::moverFocoBaixo() {
    int maxIndex = static_cast<int>(campos.size()) + 1;
    if (foco < maxIndex) {
        foco++;
    } else {
...
bool UITerminalForm::processarInput() {
